    ...
    let htmlBody = `
      <div style="font-family: Arial, sans-serif; max-width: 600px;">
        <div style="background: #1A1A2E; padding: 20px 24px; border-radius: 12px 12px 0 0;">
          <h2 style="color: #00C2E0; margin: 0; font-size: 18px;">
            ${isEnrol ? "New Enrolment Enquiry" : "Website Enquiry"}
          </h2>
          <p style="color: rgba(255,255,255,0.5); margin: 4px 0 0; font-size: 13px;">
            via baysideacademics.com.au
          </p>
        </div>
        <div style="background: #f7f8fa; padding: 24px; border: 1px solid #e5e7eb; border-radius: 0 0 12px 12px;">
          <table style="width: 100%; font-size: 14px; border-collapse: collapse;">
            <tr><td style="padding: 8px 0; color: #6b7280; width: 120px;">Name</td><td style="padding: 8px 0; font-weight: 600;">${name}</td></tr>
            <tr><td style="padding: 8px 0; color: #6b7280;">Email</td><td style="padding: 8px 0;"><a href="mailto:${email}" style="color: #00C2E0;">${email}</a></td></tr>
            ${phone ? `<tr><td style="padding: 8px 0; color: #6b7280;">Phone</td><td style="padding: 8px 0;">${phone}</td></tr>` : ""}
            ${childName ? `<tr><td style="padding: 8px 0; color: #6b7280;">Child's Name</td><td style="padding: 8px 0; font-weight: 600;">${childName}</td></tr>` : ""}
            ${yearLevel ? `<tr><td style="padding: 8px 0; color: #6b7280;">Year Level</td><td style="padding: 8px 0;">${yearLevel}</td></tr>` : ""}
            ${subject && !isEnrol ? `<tr><td style="padding: 8px 0; color: #6b7280;">Subject</td><td style="padding: 8px 0;">${subject}</td></tr>` : ""}
          </table>
          ${message ? `
